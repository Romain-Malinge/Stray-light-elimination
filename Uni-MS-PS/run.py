...
def run(model, path_obj, nb_img, folder_save, obj_name, calibrated):
    ...
    print(f"Loading images from {path_obj}...")
    imgs, mask, padding, zoom_coord, original_shape = load_imgs_mask(path=path_obj,
                                                                     nb_img=nb_img,
                                                                     calibrated=calibrated)
    
    print(f"Loaded {imgs.shape[2]} images with shape {imgs.shape[3:]}")
    ...
    print(f"Saving results to {folder_save}...")
    ...
    print(f"✓ Successfully saved: {obj_name}.png and {obj_name}.mat")